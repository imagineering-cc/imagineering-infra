...
| aiko-chat-island (Sydney / imagineering) | SQLite (messages + auth + ACL — the sole copy) | Daily 4 AM | 7 days |
| aiko-chat-island (Melbourne / enspyr) | SQLite (`aiko-island-enspyr.sql`) | Daily 4:20 AM | 7 days |
...
/opt/scripts/backup.sh aiko-island
```
...
/opt/scripts/restore.sh aiko-island
```

> ⚠️ **`aiko-island` restore replaces the SOLE store** of all island accounts,
...
| aiko-chat-island | `aiko-island.sql` | Decompressed SQLite dump |
